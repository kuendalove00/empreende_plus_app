...
const Question = ({correctOp, currentQuestionIndex = 0, handleAnswerSelect, selectedAnswer}) => {
  return(
    <div className={style.container}>
    <div className={style.questionary}>
    <p className={style.intro}>{questions[currentQuestionIndex].intro}</p>
    <p className={style.question}>{questions[currentQuestionIndex].question}</p>
    </div>
    <div  className={style.options} >
    {questions[currentQuestionIndex].options.map((option, index) => (
      <button className={`${style.option} ${option === selectedAnswer && correctOp}`} key={index} onClick={() => handleAnswerSelect(option)}>
        {option}
      </button>
    ))}
    </div>
...
const QuizPage: React.FC = () => {
  ...
  const handleAnswerSelect = (answer: string) => {
    ...
    setOptionClass(answer === questions[currentQuestionIndex].correctAnswer ? 'correct' : 'wrong')
  };
  ...
  return (
    ...
    <div className={style.main}>
      ...
      {completed == false && currentQuestionIndex < questions.length? 
        ...
        /> 
        : ""}
      <p className={style.length}>{currentQuestionIndex + 1} / {questions.length}</p>
      <PopUp
        show={showPopup}
        correct={correct}
